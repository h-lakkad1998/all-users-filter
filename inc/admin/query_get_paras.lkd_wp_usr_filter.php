...
$lkd_usr_filter_secure = (isset($_REQUEST['lkd_usr_filter_secure'])) ? wp_verify_nonce(sanitize_text_field(wp_unslash($_REQUEST['lkd_usr_filter_secure'])), 'lkd_usr_filter_secure') : false;
...
if ($lkd_usr_filter_secure) {
     ...
     $usr_sort = (isset($_REQUEST["usr_srt"])) ? sanitize_text_field(wp_unslash($_REQUEST["usr_srt"]))  : "";
     $ordr_by = (isset($_REQUEST["ordr-by"])) ? sanitize_text_field(wp_unslash($_REQUEST["ordr-by"])) : "";
     $excl_ids = (false !== $excl_ids) ? array_map('sanitize_text_field', $excl_ids) : array();
     $exlude_roles = (isset($_REQUEST["rl-excld"]) && !empty($_REQUEST["rl-excld"])) ? array_map('sanitize_text_field', wp_unslash($_REQUEST["rl-excld"])) : array();
     $cstm_dt =  (isset($_REQUEST["cstm-dt"])) ? sanitize_text_field(wp_unslash($_REQUEST["cstm-dt"]))  : "";
     $multi_from_date = $multi_to_date = array();
     if (isset($_REQUEST["mlt-f-dt"]) && isset($_REQUEST["mlt-t-dt"])) {
          $multi_from_date = (!empty($_REQUEST["mlt-f-dt"])) ? array_map('sanitize_text_field', wp_unslash($_REQUEST["mlt-f-dt"])) : [];
          $multi_to_date = (!empty($_REQUEST["mlt-t-dt"])) ? array_map('sanitize_text_field', wp_unslash($_REQUEST["mlt-t-dt"])) : [];
     }
     $one_date =  (isset($_REQUEST["one-dt"]) && !empty($_REQUEST["one-dt"]) && empty($multi_from_date)) ? date_format(date_create( sanitize_text_field( wp_unslash($_REQUEST["one-dt"]) )), "Y-m-d") : "";
     ...
     $meta_vals = (isset($_REQUEST["mta-vl"]) && is_array($_REQUEST["mta-vl"]) && !empty($_REQUEST["mta-vl"])) ? array_map("sanitize_text_field", wp_unslash($_REQUEST["mta-vl"])) : $meta_vals;
     $compatible_type = array("CHAR", "NUMERIC", "BINARY", "DATE", "DATETIME", "DECIMAL", "SIGNED", "UNSIGNED", "TIME");
     $meta_tp = (isset($_REQUEST["mta-tp"]) && is_array($_REQUEST["mta-tp"])  && !empty($_REQUEST["mta-tp"])) ? array_map("sanitize_text_field", wp_unslash($_REQUEST["mta-tp"])) : $meta_tp;
     ...
     if (isset($_REQUEST["mta-op"]) && !empty($_REQUEST["mta-op"])) {
          $meta_ops = wp_kses_post( wp_unslash($_REQUEST["mta-op"]) );
